<div class="max-w-5xl px-0.1 py-8 mx-auto sm:px-6 lg:px-8">
    ...
    </x-alert>@endif <x-card class="border shadow-sm bg-base-100 rounded-2xl border-base-200"><x-form
            wire:submit="save">
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:gap-8">
                <div class="space-y-5"><x-input label="Judul Laporan" wire:model="judul"
                        placeholder="Contoh: Jalan berlubang parah di Jl. Merdeka" required icon="o-pencil" /><x-select
                        label="Kategori" wire:model="kategori_id" :options="$kategoris" option-value="id"
                        option-label="nama" placeholder="Pilih Kategori" required icon="o-tag" /><x-textarea
                ...
                <div class="space-y-5"><x-input label="Detail Lokasi Kejadian" wire:model="lokasi_kejadian"
                    ...
                    <div class="p-5 border shadow-sm bg-base-200/30 rounded-xl border-base-200">
                        <div class="flex items-center gap-2 mb-2 text-sm font-bold text-base-content"><x-icon
                                name="o-globe-asia-australia" class="w-5 h-5 text-primary" />Koordinat Peta (Opsional)
                        </div>
                        <p class="mb-4 text-xs leading-relaxed text-base-content/60">Pilih titik lokasi pada peta untuk
                            mempermudah petugas menemukan lokasi kejadian. </p>
                        <div x-data="leafletMap()" x-init="initMap()" class="w-full relative z-0 mb-4">

                            <div class="flex flex-col sm:flex-row gap-2 mb-3">
                                <div class="relative flex-1"><x-input x-model="searchQuery"
                                        @keydown.enter.prevent="searchLocation"
                                        placeholder="Cari nama tempat / jalan..."
                                        class="w-full bg-base-100 pr-10" /><button type="button" @click="searchLocation"
                                        class="absolute right-2 top-1/2 -translate-y-1/2 btn btn-ghost btn-xs btn-circle text-base-content/50 hover:text-primary"><x-icon
                                            name="o-magnifying-glass" class="w-4 h-4" /></button></div><x-button
                        ...
                        <div class="flex items-start gap-1.5 text-[11px] text-info/80 italic"><x-icon
                                name="o-information-circle" class="w-3.5 h-3.5 shrink-0 mt-0.5" /><span>Anda dapat
                                mengklik atau menggeser peta untuk menandai lokasi secara presisi.</span></div>
                    </div>
                    <div
                        ...
                        <label class="flex items-start gap-3 cursor-pointer group">
                            <div class="relative flex items-center justify-center shrink-0 w-5 h-5 mt-0.5"><input
                                    wire:model="is_anonymous" id="is_anonymous" type="checkbox"
                                ...
                                <div
                                    ...
                                    class="absolute z-0 w-3.5 h-3.5 text-white transition-opacity opacity-0 peer-checked:opacity-100"
                            ...
                            <div class="flex flex-col"><span
                                    ...
                                    nama saya (Anonim)</span><span
                                    class="text-xs mt-0.5 leading-relaxed text-base-content/60">Nama Anda tidak akan
                                    tampil di publik, namun tetap aman tercatat di sistem admin. </span></div>
                        </label>
                    </div>
                </div>
            </div><x-slot:actions>
