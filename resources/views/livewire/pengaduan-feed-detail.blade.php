            ...
            <div class="bg-base-100 sm:rounded-2xl sm:shadow-sm border-y sm:border border-base-200 overflow-hidden">
                <div class="py-4 px-3 sm:p-6 sm:px-0 md:p-8 space-y-4 sm:space-y-6">
                    ...
                    @if($this->pengaduan->linked_id)
                        <div
                            class="mb-6 p-4 rounded-xl border border-primary/20 bg-primary/5 animate-in fade-in zoom-in duration-300">
                            <div class="flex items-start gap-3">
                                <div class="p-2 bg-primary/10 rounded-lg shrink-0 mt-0.5">
                                    <x-icon name="o-link" class="w-5 h-5 text-primary" />
                                </div>
                                <div>
                                    <h3 class="text-sm font-black text-primary mb-1">Laporan Duplikat (Selesai)</h3>
                                    <p class="text-[11px] sm:text-xs leading-relaxed text-base-content/80 font-medium">
                                        Masalah ini sudah ditangani sebelumnya. Lihat penyelesaian asli pada laporan:
                                        <a href="{{ route('pengaduan.feed-detail', $this->pengaduan->linkedReport->kode_tracking) }}"
                                            class="font-bold text-primary hover:underline">
                                            {{ $this->pengaduan->linkedReport->kode_tracking }}
                                        </a>
                                    </p>
                                </div>
                            </div>
                            ...
                            @foreach($steps as $index => $step)
                                <div class="flex flex-col items-center flex-1 relative">
                                    ...
                                    @if($index < count($steps) - 1)
                                        <div
                                            class="absolute left-1/2 top-4 w-full h-[2px] {{ isset($steps[$index + 1]) && $steps[$index + 1]['active'] ? 'bg-success' : 'bg-base-300' }} -z-0 transition-colors duration-500">
                                        </div>
                                    @endif

                                    <div
                                        class="w-8 h-8 rounded-full flex items-center justify-center transition-all duration-500 shadow-sm relative z-10
                                    ...
                                    <span
                                        class="text-[9px] sm:text-[10px] font-black mt-2 transition-colors
                    ...
                    @if($this->pengaduan->foto_bukti && count($this->pengaduan->foto_bukti) > 0)
                        <div class="mt-4 sm:mt-8">
                            ...
                            <div class="relative group">
                                ...
                                <div class="carousel w-full snap-x snap-mandatory overflow-x-auto scroll-smooth sm:rounded-2xl bg-base-200 sm:border border-base-200 no-scrollbar"
                                    id="carousel-detail">
                                    @foreach($this->pengaduan->foto_bukti as $index => $foto)
                                        <div id="detail-slide{{ $index }}"
                                            ...
                                            {{-- Navigation Overlay --}}
                                            <div
                                                class="absolute bottom-4 right-4 bg-black/60 text-white text-[10px] px-2.5 py-1 rounded-full font-bold backdrop-blur-md border border-white/10">
                                ...
                                @if(count($this->pengaduan->foto_bukti) > 1)
                                    <div
                                        class="absolute inset-y-0 left-0 flex items-center pl-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button
                                            onclick="document.getElementById('carousel-detail').scrollBy({left: -document.getElementById('carousel-detail').offsetWidth, behavior: 'smooth'})"
                                            class="btn btn-circle btn-xs sm:btn-sm bg-white/20 backdrop-blur-md border-none text-white hover:bg-white/40">❮</button>
                                    </div>
                                    <div
                                        class="absolute inset-y-0 right-0 flex items-center pr-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button
                                            onclick="document.getElementById('carousel-detail').scrollBy({left: document.getElementById('carousel-detail').offsetWidth, behavior: 'smooth'})"
                                            class="btn btn-circle btn-xs sm:btn-sm bg-white/20 backdrop-blur-md border-none text-white hover:bg-white/40">❯</button>
                                    </div>
                            ...
                            <div class="flex-1 flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs sm:text-[13px] font-bold text-base-content">
                                        {{ $this->pengaduan->tanggal_kejadian->isoFormat('D MMMM YYYY') }}
                                    </p>
                                </div>
                            </div>
                            @endif
                    ...
                    @if($this->pengaduan->harapan_pelapor)
                        <div class="p-4 bg-primary/5 border-l-4 border-primary rounded-r-xl mt-4 break-all overflow-hidden">
                            ...
                            <p class="text-sm font-bold text-base-content/80 italic">
                                "{{ $this->pengaduan->harapan_pelapor }}"</p>
                        </div>
                    @endif

                    {{-- Bukti Penyelesaian / Tindak Lanjut Instansi --}}
                    @if($this->pengaduan->status === 'selesai')
                    <div class="mt-6 border border-success/30 bg-success/5 rounded-xl sm:rounded-2xl p-3 sm:p-4">
                        <div class="flex items-center justify-between mb-3">
                            <div class="flex items-center gap-2">
                                <div class="w-6 h-6 rounded-full bg-success text-white flex items-center justify-center shadow-sm">
                                    <x-icon name="o-check-badge" class="w-3.5 h-3.5" />
                                </div>
                                <h3 class="text-xs sm:text-sm font-black text-success">Bukti Tindak Lanjut</h3>
                            </div>
                            <span class="text-[9px] sm:text-[10px] font-black uppercase text-success/60">{{ $this->pengaduan->updated_at->isoFormat('D MMMM YYYY') }}</span>
                        </div>
